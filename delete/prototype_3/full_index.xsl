...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:call-template name="naf_index_navigation">
    ...
    <xsl:template name="naf_index_navigation">
        <xsl:param name="index_xml"/>
        <xsl:param name="naf_xml"/>
        <xsl:result-document href="naf_index_navigation.html">
            <div class="navigation">
                ...
                <ul id="index">
                    ...
                    <li>
                        ...
                        <ul class="nested">
                            <xsl:for-each select="$naf_xml/nameAuthorityFile/agents/agent[@agentType='person']">
                                ...
                                <xsl:variable name="person_preferred_name" select="preferredName"/>
                                <xsl:variable name="person_id" select="@id"/>
                                <li><a href="{$person_id}.html"><xsl:value-of select="$person_preferred_name"/></a></li>
                            </xsl:for-each>
                        </ul>
                    </li>
                    ...
                    <li>
                        ...
                        <ul class="nested">
                            <xsl:for-each select="$naf_xml/nameAuthorityFile/agents/agent[@agentType='family']">
                                <xsl:sort select="preferredName"/>
                                <xsl:variable name="family_preferred_name" select="preferredName"/>
                                <xsl:variable name="family_id" select="@id"/>
                                <li><a href="{$family_id}.html"><xsl:value-of select="$family_preferred_name"/></a></li>
                            </xsl:for-each>
                        </ul>
                    </li>
                    ...
                    <li>
                        ...
                        <ul class="nested">
                            <xsl:for-each select="$naf_xml/nameAuthorityFile/agents/agent[@agentType='corporation']">
                                <xsl:sort select="preferredName"/>
                                <xsl:variable name="corporation_preferred_name" select="preferredName"/>
                                <xsl:variable name="corporation_id" select="@id"/>
                                <li><a href="{$corporation_id}.html"><xsl:value-of select="$corporation_preferred_name"/></a></li>
                            </xsl:for-each>
                        </ul>
                    </li>
                    ...
                    <li>
                        ...
                        <ul class="nested">
                            <xsl:for-each select="$index_xml/newsletterIndex/terms/term[not(parentTerm)]">
        ...
        <xsl:choose>
            <xsl:when test="$index_xml/newsletterIndex/terms/term/parentTerm/@term_id=$parent_id">
                <li>
                    ...
                    <ul class="nested">
                        <xsl:for-each select="$index_xml/newsletterIndex/terms/term[parentTerm/@term_id = $parent_id]">
    ...
    <xsl:template name="breadcrumbs">
        ...
        <xsl:choose>
            <!-- Tiers 2-6 -->
            <xsl:when test="$index_xml/newsletterIndex/terms/term[@id=$current_id and parentTerm]">
                <xsl:variable name="parent_id" select="$index_xml/newsletterIndex/terms/term[@id=$current_id]/parentTerm/@term_id"/>
                <xsl:variable name="parent_term" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id]/preferredTerm"/>
                <xsl:choose>
                    <!-- Tiers 3-6 -->
                    <xsl:when test="$index_xml/newsletterIndex/terms/term[@id=$parent_id and parentTerm]">
                        <xsl:variable name="parent_id_1" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id]/parentTerm/@term_id"/>
                        <xsl:variable name="parent_term_1" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_1]/preferredTerm"/>
                        <xsl:choose>
                            <!-- Tiers 4-6 -->
                            <xsl:when test="$index_xml/newsletterIndex/terms/term[@id=$parent_id_1 and parentTerm]">
                                <xsl:variable name="parent_id_2" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_1]/parentTerm/@term_id"/>
                                <xsl:variable name="parent_term_2" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_2]/preferredTerm"/>
                                <xsl:choose>
                                    <!-- Tiers 5-6 -->
                                    <xsl:when test="$index_xml/newsletterIndex/terms/term[@id=$parent_id_2 and parentTerm]">
                                        <xsl:variable name="parent_id_3" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_2]/parentTerm/@term_id"/>
                                        <xsl:variable name="parent_term_3" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_3]/preferredTerm"/>
                                        <xsl:choose>
                                            <!-- Tier 6 -->
                                            <xsl:when test="$index_xml/newsletterIndex/terms/term[@id=$parent_id_3 and parentTerm]">
                                                <xsl:variable name="parent_id_4" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_3]/parentTerm/@term_id"/>
                                                <xsl:variable name="parent_term_4" select="$index_xml/newsletterIndex/terms/term[@id=$parent_id_4]/preferredTerm"/>
